...
import { tryBlockByBlock } from "./block-by-block";
import { parseEvents } from "./block-reader";
import { getConfig } from "../config";
import { getParameters } from "../parameters";
import { getContract } from "../contract";
import { writeFile } from "../file-helper";
import { getLoadMode } from "./load-mode";
...
export const getEvents = async () => {
	...
	const loadMode = await LoadMode.get(symbol);

	console.log(`Load mode: ${loadMode.mode}`);

	if (loadMode.mode == "incremental" || loadMode.mode == "initial-load") {
		console.log("Resuming scanning from the last scanned block #", loadMode.blockNumber);
		fromBlock = loadMode.blockNumber + 1;
	}
