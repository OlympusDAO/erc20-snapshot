...
export const getEvents = async () => {
  ...
  const lastLoadedBlock = loadMode.lastLoadedBlock;
  ...
  // If `lastLoadedBlock` == null (which means there are no files in ./tx for this symbol), we also scan.

  // IMPORTANT NOTE: I didn't consider the case where events had already been loaded into ./tx and we are trying to scan a block interval 
  // [fromBlock, toBlock] where fromBlock is lower than lowest already loaded block index. 
  // This will fail on that case, but I'm assuming that the parameter `fromBlock` defined in `snapshot.config.json` keeps static 
  // throughout all runs and is set to be the the first block where there was a transaction for the ERC20 we're targetting.
  if (toBlock > lastLoadedBlock) {
