...
const writeInitial = async (balances) => {
  ...
  if (insertUpdateMany.errors) {
		return { status: false, message: JSON.stringify(insertUpdateMany.errors) };
  }
  ...
  if (nBalances !== affectedRows) {
	  console.log("WARNING: There were", nBalances, "addresses to insert, but", affectedRows, "were affected by the INITIAL LOAD Hasura mutation.")
  }
