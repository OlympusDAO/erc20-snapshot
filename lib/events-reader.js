...
const readEventFiles = async (directory, fileNames, loadMode) => {
  ...
  // Keep only the files until Config.toBlock
  var nFilesTotal = fileNames.length;
  fileNames = fileNames.filter(fileName => 
    parseInt(fileName.replace(".json", "")) >= Config.fromBlock && parseInt(fileName.replace(".json", "")) <= Config.toBlock);
	var nFiles = fileNames.length;
  console.log("Using", nFiles, "out of", nFilesTotal,
    "block files in ./tx to calculate final balances. (considering from block", Config.fromBlock, "to", Config.toBlock);
